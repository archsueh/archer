...
/// Pure helpers for `ArcherHook` CLI: builds the JSON payloads it ships to
...
/// `ArcherKit` on purpose — the CLI binary must remain dependency-free.
public enum ArcherHookKit {
    public static var socketPath: String {
        let support = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first!
        return support.appendingPathComponent("archer/socket").path
    }
    ...
    /// `archer-hook env ...` calling convention in `ShellIntegration.swift`'s
    ...
    /// field. Per `/plan-eng-review` D2 — ArcherHook truncates at source so
    ...
    /// fields from hook stdin JSON) and ArcherHook's `tool` argv branch (Pi —
