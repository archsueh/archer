...
public struct PanelWidths: Codable, Sendable, Equatable {
    ...
    public mutating func resize(_ kind: PanelKind, to width: Double) {
        switch kind {
        case .sidebar: sidebar = Self.clamp(width, to: Self.sidebarRange)
        case .rightPanel: rightPanel = Self.clamp(width, to: Self.rightRange)
        }
    }
}
