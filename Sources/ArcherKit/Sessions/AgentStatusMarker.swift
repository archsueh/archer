...
/// status. Unlike `ArcherHook`, this rides the terminal byte stream itself, so
/// an ssh remote can report `claude running` without reaching archer's local
/// unix socket.
///
/// Wire title shape:
///   archer-agent:<agent binary slug>:<HookEvent raw value>
///
...
enum AgentStatusMarker {
    private static let prefix = "archer-agent:"
