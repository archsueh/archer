...
/// home dir. The fallback prevents archer from spawning a shell at a deleted
...
final class WorkspaceStore {
    ...
    func setFilePanelMode(_ mode: SidebarMode) { // [archer]
        guard filePanelMode != mode else { return }
        filePanelMode = mode
        scheduleSave()
    }

    func setDiffPanelMode(_ mode: SidebarMode) { // [archer]
        guard diffPanelMode != mode else { return }
        diffPanelMode = mode
        scheduleSave()
    }
    ...
    /// to `ArcherSettingsModel.shared.agentOptions[id]`; tests pass a closure
    /// that returns nil so unit tests stay independent of the developer's
    /// real `~/.archer/settings.json`.
    private let optionsProvider: @MainActor (String) -> String?
    /// Reads `ArcherSettingsModel.shared.resumeConversations` at spawn time;
    ...
    init(
        ...
        optionsProvider: @escaping @MainActor (String) -> String? = { ArcherSettingsModel.shared.agentOptions[$0] },
    ...
    /// `git worktree remove` while archer was closed) gets dropped.
    ///
    /// v0.19.0 removed the disk → sidebar adopt path: archer no longer
    ...
    /// the single source of truth for what archer displays.
    ///
    ...
    func reconcile(source: Workspace, sourceRoot: URL? = nil, diskWorktrees: [WorktreeManager.Info]) {
        ...
        // user opens archer a lot, every microsecond on this path adds up
    ...
    func removeWorktreeDirectory(_ workspace: Workspace) async -> String? {
        ...
        let result = await Task.detached(priority: .userInitiated) {
            ...
            // `git switch`-ed inside the worktree since archer last
    ...
    func splitPaneWithExistingTab(_ pane: Pane, orientation: SplitOrientation, tabId: UUID, position: SplitPosition, in workspace: Workspace) {
           ...
           let sessionIndex = sourcePane.tabs.firstIndex(where: { $0.id == tabId })
        {
            ...
            for source in peerStores() where source !== self {
                if let session = source.surrenderSession(id: tabId) {
                    sessionToMove = session
                    break
                }
            }
        ...
        if position == .first {
            firstChild = PaneNode(pane: newPane)
            secondChild = PaneNode(pane: existing)
        } else {
    ...
    /// across archer launches. Same-value writes are dropped so we don't
    ...
    private func spawnSession(template: AgentTemplate, initialCwd: URL, sessionId: UUID = UUID(), conversationId: String? = nil, initialPrompt: String? = nil) -> Session {
        ...
        // `refreshClaudeCustomSettings`); `archerEnvironment` routes this
        // session's ARCHER_HOOKS_PATH there.
