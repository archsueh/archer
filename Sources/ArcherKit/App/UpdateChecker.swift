...
/// against `ArcherApp.displayVersion`. Manual-only for now (Help → Check
...
enum UpdateChecker {
    ...
    static func check(currentVersion: String = ArcherApp.displayVersion) async -> Outcome {
        var request = URLRequest(url: ArcherApp.releasesAPIURL)
