...
enum ArcherShellIntegration {
    ...
    ///    spilled to `~/Library/Caches/archer/pastes/screenshot-<ts>.png`,
    ...
    /// When `host` is set and the pasteboard carries a file/image payload,
    /// uploads async and hands the escaped remote path(s) to `deliver` on the
    /// main actor — fail-closed beep when the transfer fails, so a dead
    /// connection never pastes a local path the remote can't read — and
    /// returns true. False = not a remote-upload paste; the caller falls
    /// through to its local paste path.
    @discardableResult
    ...
    /// into a fresh `/tmp/archer-pastes-*` dir, resolving to the space-joined
    ...
    static func remotePasteUpload(from pb: NSPasteboard, host: String) -> (@Sendable () async -> String?)? {
        let remoteDir = "/tmp/archer-pastes-\(pasteFilenameTimestamp.string(from: Date()))-\(UUID().uuidString.prefix(8))"
    ...
    /// Connection-multiplex options shared by the archer-ssh MAIN connection
    ...
    static let sshMultiplexOptions = [
        ...
        "-o", "ControlPath=/tmp/archer-ssh-%C",
        "-o", "ControlPersist=30",
    ]
    ...
    /// mkdir for this paste plus a piggybacked expiry sweep: archer-pastes
    ...
    private static func remotePasteMkdirCommand(_ remoteDir: String) -> String {
        "find /tmp -maxdepth 1 -name 'archer-pastes-*' -type d -mmin +60 -exec rm -rf {} + 2>/dev/null; mkdir -p -- \(quote(remoteDir))"
    }
    ...
    /// fill-the-buffer deadlock class `runGit` hit.
    private static func runRemotePasteProcess(_ executable: String, _ arguments: [String], timeout: TimeInterval) -> Bool {
        ...
        do {
            ...
            NSLog("archer: remote paste could not launch %@: %@", executable, error.localizedDescription)
            return false
        }
        ...
        guard process.terminationStatus == 0 else {
            NSLog("archer: remote paste %@ exited %d", executable, process.terminationStatus)
            return false
        }
        return true
    }
    ...
    static func remotePasteDestinations(for urls: [URL], remoteDir: String) -> [(local: URL, remotePath: String)] {
    ...
    static func sanitizedRemotePasteFilename(_ raw: String) -> String {
        var out = ""
    ...
    /// Spill a pasteboard image to a archer-owned cache file. Returns
    ...
    /// Encode (TIFF → PNG when needed) and spill to the pastes cache.
    ...
    /// Lazy-created `~/Library/Caches/archer/pastes/`. Mirrors the
    /// `archerBinDirectory` / `hooksDirectory` pattern: one
    /// `createDirectory` at first access, all subsequent paste-spills
    /// skip the FS check.
    private static let pastesCacheDirectory: URL = {
        let caches = FileManager.default.urls(for: .cachesDirectory, in: .userDomainMask).first!
        let dir = caches.appendingPathComponent("archer/pastes", isDirectory: true)
    ...
    /// `~/Library/Application Support/archer/<subpath>` — single source for the
    ...
    /// `internal` (not `private`) so cross-file helpers (e.g. ClosedLidSleep's
    /// ownership marker) can reuse it without re-deriving the path.
    static func archerAppSupport(_ subpath: String, isDirectory: Bool) -> URL {
        FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first!
            .appendingPathComponent("archer/\(subpath)", isDirectory: isDirectory)
    }

    /// Directory we prepend to spawned-shell `PATH` so wrapper scripts (e.g.
    /// `claude` shim) get found before the real binaries on disk.
    static let archerBinDirectory: String = {
        let dir = archerAppSupport("bin", isDirectory: true)
        try? FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        return dir.path
    }()
    ...
    /// dedicated `archer.json` co-exists with anything the user has dropped
    /// in there. Pure path computation — the directory is materialised
    /// (and the file written) only when `~/.copilot/` already exists, so
    /// non-Copilot users don't get an empty archer-owned vendor dir in their
    ...
    static let opencodePluginPath: String = {
        ...
        try? FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        return dir.appendingPathComponent("archer.ts").path
    }()

    private static let hooksDirectory: URL = {
        let dir = archerAppSupport("hooks", isDirectory: true)
        try? FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        return dir
    }()

    /// Absolute path to the `ArcherHook` helper we exec for IPC. We do NOT run
    ...
    /// main binary does — so every build that changes ArcherHook's code (new
    ...
    /// bundled one exits 137). So copy ArcherHook into Application Support — a
    ...
    static let archerHookBinaryPath: String = {
        guard let exe = Bundle.main.executablePath else { return "" }
        let bundled = (exe as NSString).deletingLastPathComponent + "/ArcherHook"
        ...
        // the test suite can't clobber the helper a live archer depends on.
    ...
    /// `archerBinDirectory` so wrapper shims resolve before the real binaries.
    /// `claudeCustomSettingsAgentId`, when set, routes `ARCHER_HOOKS_PATH` to
    /// that custom agent's per-agent Claude settings file (endpoint / key)
    /// instead of the shared `claude.json`.
    static func archerEnvironment(
        for sessionId: UUID,
        ...
        var env: [String: String] = [
            "ARCHER_SURFACE_ID": sessionId.uuidString,
            "ARCHER_HOOKS_PATH": hooksPath,
            "ARCHER_BIN_DIR": archerBinDirectory,
            "ARCHER_HOOK_BIN": archerHookBinaryPath,
            // ARCHER_AGENT_MARKERS is deliberately NOT set locally: the
            // ArcherHook socket is the local status channel. OSC-title markers
            ...
            "PATH": "\(archerBinDirectory):\(parentPath)",
        ...
        if let original = ProcessInfo.processInfo.environment["ZDOTDIR"], !original.isEmpty {
            env["ARCHER_ORIGINAL_ZDOTDIR"] = original
        }
        return env
    }
    ...
    /// tracks the latest `ArcherHook` location.
    static func installAgentHooks(sshRemoteAgentDetection: Bool = false) {
        ...
        // unlocks the `--` remote-agent protocol and connection multiplexing.
        writeWrapper(name: "archer-ssh", script: sshWrapperScript)
        ...
        // Gemini we can't point it at a archer-owned defaults file, and unlike
        // Copilot it has no per-event hooks directory; the bracket wrapper
        // gives running/ended until a config.toml-merge path exists.
        //
        // Pi rides a archer-managed TypeScript extension (installed only when
        // `~/.pi/` exists — see `installPiExtensionIfPresent`) that subscribes
        // to pi's session / turn events and pings ArcherHook, same model as the
        ...
        // lifecycle feed archer can map to attention, so running/ended is all
    ...
    static func refreshSshRemoteAgentDetection(enabled: Bool) {
        if enabled {
            ...
            removeManagedWrapper(
                name: "ssh",
                markers: ["ARCHER_DISABLE_SSH_AGENT_MARKERS", "archer-agent-markers"]
            )
        }
    }
    ...
    /// Skips otherwise so archer doesn't pre-stage a vendor namespace for
    /// users who may never install Copilot. Installing Copilot later then
    /// requires one archer restart to pick up the hooks (acceptable: the
    ...
    /// run, and a archer restart picks up the extension once `~/.pi/` exists.
    ...
    /// OpenCode plugin: gated on `ARCHER_SURFACE_ID`, reads `ARCHER_HOOK_BIN`
    /// from the env archer injects, and carries the managed marker so a user
    /// edit isn't clobbered. Pi runs extensions under Node, so `process.env`
    /// and `pi.exec` are available.
    static let piExtensionScript = """
    // \(managedFileMarker) — pings ArcherHook on pi's session / turn / tool
    ...
    // events), and the session id is reported so archer can resume the
    // conversation (`pi --session <id>`) after a restart. Safe to delete; it
    // is regenerated next time archer launches.
    export default function (pi) {
      ...
      // is stable across the pair, so archer matches start/end by it.
    ...
    /// for that agent's sessions, overriding `ARCHER_HOOKS_PATH`.
    ...
    /// only — archer never exports them to the shell.
    ...
    /// stdout — `ArcherHook` only writes to its socket so this is safe.
    ...
    /// names → lifecycle state (running / attention / idle / ended); archer-hook
    ...
    /// Claude's `PreToolUse` / `PostToolUse` — archer-hook reads stdin for
    ...
    private static func hooksObject(
        ...
        // `ArcherHook` path breaks the moment the app lives under a path with
        // spaces or shell metacharacters (e.g. `/Applications/Archer 2.app/…`).
        let quotedCmd = quote(hookCmd)
        for (event, state) in events {
    ...
    /// Marker we embed at the top of every archer-generated user-config file
    ...
    private static let managedFileMarker = "archer-managed-do-not-edit"
    ...
    /// `archer.json` that happens to live at the same path by looking for the
    /// `_archerManaged` sentinel field. The Copilot hooks dir is user-owned
    ...
    /// `ARCHER_AGENT_MARKERS` is set — ssh remotes export it, local sessions
    /// don't (they report through the ArcherHook socket), so the local bracket
    ...
    private static func agentMarkerCommand(slug: String, event: HookEvent) -> String {
        "[[ -n \"$ARCHER_AGENT_MARKERS\" ]] && printf '\\033]2;\(AgentStatusMarker.title(slug: slug, event: event))\\a' 2>/dev/null > /dev/tty"
    }
    ...
    private static func wrapperPreamble(binary: String) -> String {
        ...
        if [[ -z "$real" ]]; then
            ...
            if [[ -n "$ARCHER_SURFACE_ID" && -n "$ARCHER_HOOK_BIN" ]]; then
                "$ARCHER_HOOK_BIN" \(binary) ended 2>/dev/null
            fi
            exit 127
        fi
        """
    ...
    /// straight through, skipping instrumentation: a ArcherHook ping, OSC
    ...
    /// Inside a archer session ($ARCHER_SURFACE_ID set), injects --settings so
    /// Claude Code's hooks report state back to the app via the bundled
    /// ArcherHook helper. `ARCHER_AGENT_MARKERS` enables the OSC-title fallback
    ...
    if [[ -n "$ARCHER_SURFACE_ID" || -n "$ARCHER_AGENT_MARKERS" ]]; then
        ...
        status=$?
        if [[ -n "$ARCHER_SURFACE_ID" && -n "$ARCHER_HOOK_BIN" ]]; then
            "$ARCHER_HOOK_BIN" codex ended 2>/dev/null
        fi
    ...
    /// SSH is the one common path where the agent runs outside archer's local
    ...
    ///
    /// The same script is installed as both `ssh` (user-setting gated) and
    /// private `archer-ssh` (always installed for SSH workspaces). Invoked
    /// as `archer-ssh`, it additionally: (1) accepts `archer-ssh host --
    /// <agent argv…>` for remote agent launch, (2) enables ControlMaster
    /// multiplexing shared with paste upload, (3) waits for ssh and emits
    /// a logout marker so `remoteHost` clears only when the connection ends.
    static let sshWrapperScript: String = {
        let remoteCommand = "sh -lc \(quote(remoteAgentBootstrapScript))"
        return """
        \(wrapperPreamble(binary: "ssh"))

        if [[ -n "${ARCHER_DISABLE_SSH_AGENT_MARKERS:-}" || ! -t 0 || ! -t 1 ]]; then
            exec "$real" "$@"
        fi
        ...
        for ((i = 0; i < ${#args[@]}; i++)); do
            ...
            # archer-ssh only: `archer-ssh <dest> -- <agent argv…>` asks the
        ...
        if (( ${#remote_agent_args[@]} )); then
            ...
            printf -v _archer_remote_agent '%q ' "${remote_agent_args[@]}"
            printf -v _archer_remote_agent_q '%q' "${_archer_remote_agent% }"
            remote_command="env ARCHER_REMOTE_AGENT=$_archer_remote_agent_q $remote_command"
        fi
        _archer_mux_opts=()
        if [[ "${0##*/}" == "archer-ssh" ]]; then
            # archer-owned connections multiplex (ControlPath shared with the
            ...
            # ssh onto shared connections is not archer's call to make.
            _archer_mux_opts=(\(sshMultiplexOptions.joined(separator: " ")))
        fi
        ...
        trap '' INT QUIT
        "$real" -t "${_archer_mux_opts[@]}" "${args[@]}" "$remote_command"
        _archer_ssh_status=$?
        printf '\\033]2;\(RemoteLoginMarker.logoutTitle)\\a' > /dev/tty 2>/dev/null
        exit "$_archer_ssh_status"
        """
    }()

    /// Consume-once eval of the agent command the archer-ssh wrapper passed
    /// via the `ARCHER_REMOTE_AGENT` env prefix — interpolated into all three
    /// bootstrap shell branches. `heredocEscaped: true` renders the `\$` form
    /// the zsh/bash rc heredocs need (the OUTER bootstrap shell must leave
    ...
    private static func remoteAgentEvalBlock(heredocEscaped: Bool) -> String {
        ...
        return """
        if [ -n "\(d){ARCHER_REMOTE_AGENT:-}" ]; then
            _archer_remote_agent="\(d)ARCHER_REMOTE_AGENT"
            unset ARCHER_REMOTE_AGENT
            eval "\(d)_archer_remote_agent"
        fi
        """
    }
    ...
    static let remoteAgentBootstrapScript: String = {
        ...
        return #"""
        _archer_root="${TMPDIR:-/tmp}/archer-agent-markers-${USER:-user}-$$"
        _archer_bin="$_archer_root/bin"
        mkdir -p "$_archer_bin" 2>/dev/null || {
            printf 'archer: could not create remote marker directory\n' >&2
            "${SHELL:-/bin/sh}" -l
            exit $?
        }
        trap 'rm -rf "$_archer_root"' EXIT
        trap 'rm -rf "$_archer_root"; exit' HUP INT TERM

        _archer_write_agent_wrapper() {
            _archer_slug="$1"
            cat > "$_archer_bin/$_archer_slug" <<'ARCHER_AGENT_WRAPPER'
        #!/bin/sh
        _archer_slug="${0##*/}"
        _archer_self_dir=$(CDPATH= cd "$(dirname "$0")" && pwd)
        _archer_real=""
        _archer_old_ifs=$IFS
        IFS=:
        for _archer_dir in $PATH; do
            [ "$_archer_dir" = "$_archer_self_dir" ] && continue
            [ -x "$_archer_dir/$_archer_slug" ] || continue
            _archer_real="$_archer_dir/$_archer_slug"
            break
        done
        IFS=$_archer_old_ifs

        if [ -z "$_archer_real" ]; then
            printf '\033]2;archer-agent:%s:ended\a' "$_archer_slug" > /dev/tty 2>/dev/null
            printf '\n  %s is not installed.\n\n' "$_archer_slug" >&2
            exit 127
        fi

        printf '\033]2;archer-agent:%s:running\a' "$_archer_slug" > /dev/tty 2>/dev/null
        "$_archer_real" "$@"
        _archer_status=$?
        printf '\033]2;archer-agent:%s:ended\a' "$_archer_slug" > /dev/tty 2>/dev/null
        exit "$_archer_status"
        ARCHER_AGENT_WRAPPER
            chmod +x "$_archer_bin/$_archer_slug"
        }

        for _archer_slug in \#(slugs); do
            _archer_write_agent_wrapper "$_archer_slug"
        done
        unset _archer_slug

        case "${SHELL:-}" in
            */zsh)
                mkdir -p "$_archer_root/zsh"
                cat > "$_archer_root/zsh/.zshrc" <<ARCHER_ZSHRC
        if [[ -n "\${ARCHER_ORIGINAL_ZDOTDIR:-}" ]]; then
            export ZDOTDIR="\$ARCHER_ORIGINAL_ZDOTDIR"
            unset ARCHER_ORIGINAL_ZDOTDIR
        else
            unset ZDOTDIR
        fi
        ...
        export ARCHER_AGENT_MARKERS=1
        export PATH="$_archer_bin:\$PATH"
        \#(remoteAgentEvalBlock(heredocEscaped: true))
        ARCHER_ZSHRC
                ARCHER_ORIGINAL_ZDOTDIR="${ZDOTDIR:-}" ZDOTDIR="$_archer_root/zsh" zsh -l
                ;;
            */bash)
                cat > "$_archer_root/bashrc" <<ARCHER_BASHRC
        _archer_login_rc_loaded=
        for _archer_rc in "\$HOME/.bash_profile" "\$HOME/.bash_login" "\$HOME/.profile"; do
            if [[ -r "\$_archer_rc" ]]; then
                source "\$_archer_rc"
                _archer_login_rc_loaded=1
                break
            fi
        done
        unset _archer_rc
        if [[ -z "\$_archer_login_rc_loaded" && -r "\$HOME/.bashrc" ]]; then
            source "\$HOME/.bashrc"
        fi
        unset _archer_login_rc_loaded
        export ARCHER_AGENT_MARKERS=1
        export PATH="$_archer_bin:\$PATH"
        \#(remoteAgentEvalBlock(heredocEscaped: true))
        ARCHER_BASHRC
                bash --rcfile "$_archer_root/bashrc" -i
                ;;
            *)
                export ARCHER_AGENT_MARKERS=1
                export PATH="$_archer_bin:$PATH"
    ...
    case "$resolved" in
        */Antigravity.app/*)
            ...
            if [[ -n "$ARCHER_SURFACE_ID" && -n "$ARCHER_HOOK_BIN" ]]; then
                "$ARCHER_HOOK_BIN" agy ended 2>/dev/null
            fi
            exit 127
            ;;
    esac
    ...
    /// and `antigravityWrapperScript`. Outside a archer session (and without
    /// `ARCHER_AGENT_MARKERS`) the bracket is a no-op — `exec "$real"` is the
    ...
    private static func bracketBody(slug: String) -> String {
        ...
        if [[ -n "$ARCHER_SURFACE_ID" || -n "$ARCHER_AGENT_MARKERS" ]]; then
            ...
            if [[ -n "$ARCHER_SURFACE_ID" && -n "$ARCHER_HOOK_BIN" ]]; then
                "$ARCHER_HOOK_BIN" \(slug) running 2>/dev/null
            fi
            "$real" "$@"
            status=$?
            if [[ -n "$ARCHER_SURFACE_ID" && -n "$ARCHER_HOOK_BIN" ]]; then
                "$ARCHER_HOOK_BIN" \(slug) ended 2>/dev/null
            fi
    ...
    /// ARCHER_SURFACE_ID + ARCHER_HOOK_BIN from the shell, and shells out to
    /// ArcherHook on each lifecycle event. The first-line marker
    /// (`managedFileMarker`) lets `writeManagedFile` recognise the file as
    /// archer-generated on upgrade — a user's own `archer.ts` plugin would
    /// not carry the marker and stays untouched.
    static let opencodePluginScript = """
    // \(managedFileMarker) — pings ArcherHook on prompt-submit and turn-end so
    // the sidebar agent dot tracks per-session activity. Safe to delete; will
    // be regenerated next time archer launches.
    export const ArcherPlugin = async ({ $ }) => {
      const surface = process.env.ARCHER_SURFACE_ID
      const hookBin = process.env.ARCHER_HOOK_BIN
      if (!surface || !hookBin) return {}
    ...
    static let bashLauncherPath: String = {
        ...
        let launcherPath = dir.appending("archer-bash-launch-\(getpid()).sh")
        let rcfilePath = dir.appending("archer-bashrc-\(getpid())")
        ...
        # User rc may rewrite PATH; re-prepend the archer wrapper directory so
        # `claude` etc. resolve to our shims first.
        [[ -n "$ARCHER_BIN_DIR" ]] && export PATH="$ARCHER_BIN_DIR:$PATH"

        _archer_osc7_pwd() { printf '\\e]7;file://%s%s\\e\\\\' "$HOSTNAME" "$PWD"; }
        ...
        _archer_title_pwd() { printf '\\e]2;%s\\a' "$PWD"; }
        \(envStatusBlock)

        PROMPT_COMMAND="_archer_title_pwd;_archer_osc7_pwd;_archer_env_status${PROMPT_COMMAND:+;$PROMPT_COMMAND}"
        _archer_osc7_pwd
        _archer_env_status
    ...
    static let zshDirectory: String = {
        let dir = NSTemporaryDirectory().appending("archer-zsh-\(getpid())")
        ...
        # Ctrl/Alt+arrow sequences to ESC+B/F so users don't notice. archer
        ...
        # archer-zsh-<pid> dir. Also stops `curl | bash`-style installers
        ...
        # still wins (it runs after). archer maps a cwd-shaped title to the
        # bare basename. `return $_s` keeps $? intact for the user hooks.
        autoload -Uz add-zsh-hook
        _archer_title_pwd() { local _s=$?; printf '\\e]2;%s\\a' "$PWD"; return $_s }
        add-zsh-hook precmd _archer_title_pwd
        ...
        _archer_osc7_pwd() { printf '\\e]7;file://%s%s\\e\\\\' "$HOST" "$PWD" }
        add-zsh-hook chpwd _archer_osc7_pwd
        _archer_osc7_pwd
    ...
    /// fish integration, auto-loaded from a archer-owned `vendor_conf.d/archer.fish`
    ...
    set -g __archer_host (hostname 2>/dev/null)
    ...
    function __archer_prepend_path
        test -n "$ARCHER_BIN_DIR"; or return
        ...
        test "$PATH[1]" = "$ARCHER_BIN_DIR"; and return
        set -gx PATH "$ARCHER_BIN_DIR" (string match -v -- "$ARCHER_BIN_DIR" $PATH)
    end
    ...
    function __archer_prompt --on-event fish_prompt
        __archer_prepend_path
        printf '\\e]7;file://%s%s\\e\\\\' "$__archer_host" "$PWD"
    end
    ...
    function __archer_env_status --on-event fish_prompt
        if test -z "$ARCHER_SURFACE_ID"; or test -z "$ARCHER_HOOK_BIN"; or not test -x "$ARCHER_HOOK_BIN"
            return
        end
        set -l node_path ""
        if command -q node
        ...
        if test "$node_key" != "$__archer_node_key_last"
            set -g __archer_node_version_last ""
            if test -n "$node_path"
                set -g __archer_node_version_last ($node_path --version 2>/dev/null)
            end
            set -g __archer_node_key_last "$node_key"
        end
        ...
        set -l env_now "$VIRTUAL_ENV|$CONDA_DEFAULT_ENV|$NVM_BIN|$NVM_DIR|$__archer_node_version_last|$https_v|$http_v|$all_v"
        if test "$env_now" = "$__archer_env_last"
            return
        end
        ...
        if "$ARCHER_HOOK_BIN" env "$VIRTUAL_ENV" "$CONDA_DEFAULT_ENV" "$NVM_BIN" "$NVM_DIR" "$__archer_node_version_last" "$https_v" "$http_v" "$all_v" 2>/dev/null
            set -g __archer_env_last "$env_now"
        end
    end

    # OSC 133 prompt/command markers — fish 4+ emits these natively, so only add
    # them on fish 3.x to avoid double-marking. Gives archer per-command exit
    # status (the failed-command red dot), duration, and jump-to-prompt.
    set -l __archer_major (string split '.' -- $version)[1]
    if test "$__archer_major" -lt 4 2>/dev/null
        function __archer_133_prompt --on-event fish_prompt
            printf '\\e]133;A;cl=line\\a'
        end
        function __archer_133_preexec --on-event fish_preexec
            printf '\\e]133;C\\a'
        end
        function __archer_133_postexec --on-event fish_postexec
    ...
    function __archer_agent_launch --on-event fish_prompt
        functions -e __archer_agent_launch
        test -n "$ARCHER_AGENT"; and test -z "$ARCHER_AGENT_LAUNCHED"; or return
        set -gx ARCHER_AGENT_LAUNCHED 1
        __archer_prepend_path
        set -l _archer_cmd $ARCHER_AGENT
        set -e ARCHER_AGENT
        set -l _archer_bin (string split -m1 ' ' -- $_archer_cmd)[1]
        # `eval` lets ARCHER_AGENT carry multi-word commands (resume flag, prompt,
        ...
        if test -n "$ARCHER_SURFACE_ID"; and test -n "$ARCHER_HOOK_BIN"
            "$ARCHER_HOOK_BIN" $_archer_bin ended 2>/dev/null
        end
    end
    """

    /// `XDG_DATA_DIRS` entry we hand fish sessions so it discovers our
    /// `fish/vendor_conf.d/archer.fish` (see `fishInitScript`). A stable
    ...
    static let fishVendorDataRoot: String = archerAppSupport("share", isDirectory: true).path

    /// Absolute path to the archer-owned fish vendor conf file.
    static let fishVendorConfPath: String = (fishVendorDataRoot as NSString).appendingPathComponent("fish/vendor_conf.d/archer.fish")
    ...
    ///
    /// Sweeps every `archer-<slug>-<pid>` artifact (optional extension) by
    ...
    /// process: another live archer's files (or a pid that's merely a digit
    ...
    static func cleanup() {
        ...
        for entry in entries where entry.hasPrefix("archer-") {
    ...
    /// Inline agent launch — invoked by both wrapper rcs to start ARCHER_AGENT
    ...
    ///     still valid.
    /// (b) the `archer-hook env` IPC fork is skipped entirely when no env key
