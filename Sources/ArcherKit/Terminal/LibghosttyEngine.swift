...
final class LibghosttyApp {
    ...
    private init() {
        ...
        var runtime = ghostty_runtime_config_s(
            ...
            wakeup_cb: archerWakeupCb,
            action_cb: archerActionCb,
            read_clipboard_cb: archerReadClipboardCb,
            confirm_read_clipboard_cb: archerConfirmReadClipboardCb,
            write_clipboard_cb: archerWriteClipboardCb,
            close_surface_cb: archerCloseSurfaceCb
        )
    ...
    func reloadConfig() {
        ...
        // share the parsed archer-side dict to avoid N+1 disk reads + JSON
...
private let archerWakeupCb: ghostty_runtime_wakeup_cb = { _ in
...
private let archerActionCb: ghostty_runtime_action_cb = { _, target, action in
    ...
    case GHOSTTY_ACTION_SHOW_CHILD_EXITED:
        // exit 0 → user typed `exit` / `logout`; auto-close the archer tab
...
private let archerReadClipboardCb: ghostty_runtime_read_clipboard_cb = { _, _, _ in false }
private let archerConfirmReadClipboardCb: ghostty_runtime_confirm_read_clipboard_cb = { _, _, _, _ in }
private let archerWriteClipboardCb: ghostty_runtime_write_clipboard_cb = { _, kind, contents, count, _ in
...
final class GhosttySurfaceView: NSView {
    ...
    override func performDragOperation(_ sender: any NSDraggingInfo) -> Bool {
        guard let urls = sender.draggingPasteboard.readObjects(forClasses: [NSURL.self]) as? [URL],
              let escaped = ArcherShellIntegration.backslashEscapedFileURLs(urls)
        else { return false }
        paste(escaped)
        return true
    }
    ...
    func createSurfaceIfReady() {
        ...
        // populates ARCHER_AGENT here so the wrapper .zshrc auto-launches the
        // selected CLI before the user ever sees a shell prompt.
        var envDict = config.environment
        envDict[ArcherShellIntegration.zdotdirKey] = ArcherShellIntegration.zshDirectory
        ...
        guard let new else {
            NSLog("archer: ghostty_surface_new failed")
            return
        }
        surface = new
        pendingConfig = nil
    ...
    func updateConfigFromSettings(parsed: [String: Any]?) {
        ...
        guard let config = ArcherSettings.makeGhosttyConfig(parsed: parsed) else {
            NSLog("archer: ghostty_config_new failed during surface reload")
            return
        }
    ...
    override func keyDown(with event: NSEvent) {
        ...
        // Archer-specific functional keys with explicit byte behavior. Skipped
    ...
    /// are post-translation control sequences archer already encodes itself.
    ...
    /// Map archer's own functional-key policy to bytes. Returns `nil` for
