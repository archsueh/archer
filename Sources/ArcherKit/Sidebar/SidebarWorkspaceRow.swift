    ...
    var body: some View {
        ...
        .popover(isPresented: $isContextMenuOpen, arrowEdge: .trailing) {
            VStack(alignment: .leading, spacing: 0) {
                ...
                ArcherMenuRow(title: "Close Other Workspaces", isDisabled: !canCloseOthers) {
                    isContextMenuOpen = false
                    onCloseOthers()
                }
                ArcherMenuDivider()
                ArcherMenuRow(title: "Rename Workspace…", shortcut: "⌘⇧R") {
                    isContextMenuOpen = false
                    beginRename(deferred: true)
                }
                ArcherMenuRow(title: "Duplicate Workspace") {
                    isContextMenuOpen = false
                    onDuplicate()
                }
                if let onCreateWorktree {
                    ArcherMenuRow(title: "Create Worktree…") {
                        isContextMenuOpen = false
        ...
        .popover(isPresented: $isRenameOpen, arrowEdge: .trailing) {
            ArcherRenameField(placeholder: "Workspace title", text: $pendingRename) {
