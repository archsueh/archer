...
final class ArcherHookKitTests: XCTestCase {
    ...
    func testBuildEnvPayloadFullArgs() {
        let payload = ArcherHookKit.buildEnvPayload(
            surface: "abc-123",
            args: [
                "/path/venv",
                "myenv",
        ...
        XCTAssertEqual(payload["ARCHER_NODE_VERSION"], "v20.11.0")
    ...
    func testBuildEnvPayloadShortArgsFillsBlanks() {
        ...
        let payload = ArcherHookKit.buildEnvPayload(surface: "s", args: ["/venv", "myenv"])
        XCTAssertEqual(payload["VIRTUAL_ENV"], "/venv")
        XCTAssertEqual(payload["CONDA_DEFAULT_ENV"], "myenv")
        XCTAssertEqual(payload["NVM_BIN"], "")
        XCTAssertEqual(payload["NVM_DIR"], "")
        XCTAssertEqual(payload["ARCHER_NODE_VERSION"], "")
    ...
    func testBuildEnvPayloadEmptyArgs() {
        let payload = ArcherHookKit.buildEnvPayload(surface: "s", args: [])
        XCTAssertEqual(payload["kind"], "env")
    ...
    func testBuildLifecyclePayload() {
        let payload = ArcherHookKit.buildLifecyclePayload(
            agent: "claude",
            event: "running",
            surface: "abc"
        )
    ...
    func testParseClaudeConversationIdValid() {
        ...
        XCTAssertEqual(ArcherHookKit.parseClaudeConversationId(from: data), "sess_abc123")
    }

    func testParseClaudeConversationIdMissingField() {
        let json = #"{"transcript_path":"/tmp/x"}"#
        XCTAssertNil(ArcherHookKit.parseClaudeConversationId(from: Data(json.utf8)))
    }
    ...
    func testParseToolEventBashPre() {
        ...
        """#
        let payload = ArcherHookKit.parseToolEventPayload(from: Data(json.utf8), surface: "abc", agent: "claude")
    ...
    func testParseToolEventRejectsMissingHookEventName() {
        ...
        XCTAssertNil(ArcherHookKit.parseToolEventPayload(from: Data(json.utf8), surface: "s", agent: "claude"))
    }
    ...
    func testParseToolEventRejectsMalformedJSON() {
        let data = Data("not valid json {{{".utf8)
        XCTAssertNil(ArcherHookKit.parseToolEventPayload(from: data, surface: "s", agent: "claude"))
    }
    ...
    func testExtractIdentifierUnknownToolWithMultipleKeys() {
        ...
        let id = ArcherHookKit.extractIdentifier(
            toolName: "Mystery",
    ...
    func testDetectSuccessHandlesNonString() {
        ...
        XCTAssertTrue(ArcherHookKit.detectSuccess(toolResponse: nil))
        XCTAssertTrue(ArcherHookKit.detectSuccess(toolResponse: 42))
        XCTAssertTrue(ArcherHookKit.detectSuccess(toolResponse: ["nested": "value"]))
    }
    ...
    func testDetectSuccessNoMarkers() {
        XCTAssertTrue(ArcherHookKit.detectSuccess(toolResponse: "all good output here"))
        XCTAssertTrue(ArcherHookKit.detectSuccess(toolResponse: ""))
    }

    // MARK: buildToolEventPayload (shared by Claude parser + Pi extension feed)

    func testBuildToolEventPayloadPre() {
        let p = ArcherHookKit.buildToolEventPayload(
