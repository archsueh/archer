...
/// real user config files. Self-heals on next archer launch but better
/// avoided: the writers are trivial, the content getters are the
/// load-bearing surface.
final class ShellIntegrationTests: XCTestCase {
    private static let stubHook = "/usr/local/bin/ArcherHook"

    func testGeminiDefaultsExposesAllFourLifecycleEvents() throws {
        let object = ArcherShellIntegration.geminiDefaultsObject(hookCmd: Self.stubHook)
    ...
    func testBracketWrapperPassesThroughWhenSurfaceIdMissing() {
        let script = ArcherShellIntegration.bracketWrapperScript(slug: "amp")

        XCTAssertTrue(script.contains("self_dir"), "must skip own dir on PATH walk")
        XCTAssertTrue(script.contains("\"$ARCHER_HOOK_BIN\" amp running"))
        XCTAssertTrue(script.contains("\"$ARCHER_HOOK_BIN\" amp ended"))
        XCTAssertTrue(script.contains("archer-agent:amp:running"))
        XCTAssertTrue(script.contains("archer-agent:amp:ended"))
        XCTAssertTrue(script.contains("ARCHER_AGENT_MARKERS"))
        ...
        XCTAssertTrue(script.contains("[[ -n \"$ARCHER_AGENT_MARKERS\" ]] && printf"), "marker gated on ARCHER_AGENT_MARKERS so local sessions stay socket-only")
    ...
    func testWrapperPassesThroughForBackgroundPipedCaller() {
        ...
        // instrumentation runs, so the wrapper never pings ArcherHook.
        let script = ArcherShellIntegration.bracketWrapperScript(slug: "amp")
    ...
    func testCodexWrapperGuardsBackgroundCallBeforeInstrumenting() {
        ...
        let script = ArcherShellIntegration.codexWrapperScript
    ...
    func testAgentStatusMarkerParsesKnownAgentTitle() throws {
        let parsed = try XCTUnwrap(AgentStatusMarker.parseTitle("archer-agent:codex:attention"))
    ...
    func testSshWrapperInjectsRemoteBootstrapForPlainInteractiveLogin() {
        ...
        XCTAssertTrue(script.contains("ARCHER_DISABLE_SSH_AGENT_MARKERS"))
    ...
    func testRemoteAgentBootstrapWritesMarkerWrappers() {
        let script = ArcherShellIntegration.remoteAgentBootstrapScript

        XCTAssertTrue(script.contains(#"_archer_root="${TMPDIR:-/tmp}/archer-agent-markers-"#))
        XCTAssertTrue(script.contains("for _archer_slug in 'claude' 'codex'"))
        ...
        XCTAssertTrue(script.contains(#"printf '\033]2;archer-agent:%s:running\a'"#))
        XCTAssertTrue(script.contains(#"printf '\033]2;archer-agent:%s:ended\a'"#))
        XCTAssertTrue(script.contains("export ARCHER_AGENT_MARKERS=1"))
        XCTAssertTrue(script.contains(#"export PATH="$_archer_bin:$PATH""#))
    ...
    func testAntigravityWrapperBracketsRunningAndEndedForRealCLI() {
        ...
        XCTAssertTrue(script.contains("\"$ARCHER_HOOK_BIN\" agy running"))
        XCTAssertTrue(script.contains("exec \"$real\" \"$@\""), "must passthrough when ARCHER_SURFACE_ID is unset")
    }

    func testOpencodePluginShellsOutToHookBinForBothEvents() {
        let body = ArcherShellIntegration.opencodePluginScript
        ...
        XCTAssertTrue(body.contains("opencode"), "plugin must pass agent slug to ArcherHook")
        XCTAssertTrue(body.contains("ARCHER_SURFACE_ID"))
        XCTAssertTrue(body.contains("archer-managed-do-not-edit"), "plugin must carry the upgrade-safety marker")
    }

    func testPiExtensionSubscribesLifecycleEventsAndPingsHook() {
        let body = ArcherShellIntegration.piExtensionScript

        // Subscribes to pi's session / turn lifecycle and maps each to a
        // ArcherHook state — running while a turn runs, attention when it ends.
        ...
        XCTAssertTrue(body.contains(#"pi.exec(hookBin, ["pi""#), "must ping ArcherHook with the pi slug")
        // Reports the session id so archer can resume (`pi --session <id>`).
        ...
        XCTAssertTrue(body.contains("ARCHER_SURFACE_ID"))
        XCTAssertTrue(body.contains("ARCHER_HOOK_BIN"))
        XCTAssertTrue(body.contains("archer-managed-do-not-edit"), "must carry the upgrade-safety marker")
    }

    func testPiExtensionReportsToolCallsForActivityPill() {
        let body = ArcherShellIntegration.piExtensionScript
        // Subscribes to pi's tool lifecycle and relays each to ArcherHook's
    ...
    func testAgentLaunchBlockRevertsIconAfterAgentReturns() {
        let block = ArcherShellIntegration.agentLaunchBlock
        ...
        XCTAssertTrue(block.contains("eval \"$_archer_cmd\""))
        XCTAssertTrue(block.contains(#"_archer_agent_bin="${_archer_cmd%% *}""#), "must derive the agent binary for the revert ping")
        XCTAssertTrue(block.contains(#""$ARCHER_HOOK_BIN" "$_archer_agent_bin" ended"#), "must ping ended after the agent returns")
        ...
        XCTAssertTrue(block.contains("_archer_status=$?"), "must capture the agent exit status before the revert ping")
        XCTAssertTrue(block.contains("( exit $_archer_status )"), "must restore the agent exit status after the ping")
    }

    func testEnvStatusBlockReportsLiveShellEnvironment() {
        let body = ArcherShellIntegration.envStatusBlock

        XCTAssertTrue(body.contains("\"$ARCHER_HOOK_BIN\" env"))
    ...
    func testBackslashEscapePassesThroughNonAscii() {
        ...
        XCTAssertEqual(ArcherShellIntegration.backslashEscape("/tmp/项目/🚀.md"), "/tmp/项目/🚀.md")
    }

    func testClaudeCustomSettingsObjectCarriesHooksAndEnv() throws {
        let object = ArcherShellIntegration.claudeCustomSettingsObject(
        ...
        // file passed to that session, so archer's activity hooks have to be
    ...
    func testBackslashEscapeFallsBackToQuoteOnNewlineToAvoidLineContinuation() {
        ...
        // literal newline.
        let escaped = ArcherShellIntegration.backslashEscape("/tmp/multi\nline/file.txt")
    ...
    private func makeIsolatedPasteboard() -> NSPasteboard {
        let unique = "archer-test-\(UUID().uuidString)"
    ...
    func testReadTerminalPasteTextReturnsRawStringForPlainText() {
        ...
        XCTAssertEqual(ArcherShellIntegration.readTerminalPasteText(from: pb), "ls -la")
    }
    ...
    func testReadTerminalPasteTextReturnsEscapedPathForFileURL() {
        ...
        XCTAssertEqual(
            ArcherShellIntegration.readTerminalPasteText(from: pb),
    ...
    func testReadTerminalPasteTextSpillsPNGImageDataToCacheFile() throws {
        ...
        // ~/Library/Caches/archer/pastes/screenshot-*.png and paste the
        // escaped file path.
        let pb = makeIsolatedPasteboard()
        pb.declareTypes([.png], owner: nil)
        pb.setData(Self.oneByOnePNG, forType: .png)
        let pasted = try XCTUnwrap(ArcherShellIntegration.readTerminalPasteText(from: pb))
        ...
        XCTAssertTrue(rawPath.contains("/archer/pastes/screenshot-"))
    ...
    func testReadTerminalPasteTextReturnsNilForEmptyPasteboard() {
        let pb = makeIsolatedPasteboard()
        pb.clearContents()
        XCTAssertNil(ArcherShellIntegration.readTerminalPasteText(from: pb))
    }
