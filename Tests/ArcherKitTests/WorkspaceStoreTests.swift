...
final class WorkspaceStoreTests: XCTestCase {
    ...
    /// Two independent stores wired as each other's peers — models two archer
    ...
    func testReconcileClosesSidebarOnlyZombies() {
        // The user `git worktree remove`-d it in a shell — archer's row
    ...
    func testPerformCloseSourceAlsoDeleteFalseSkipsGitRemoveEntirely() async {
        ...
        let fakePath = FileManager.default.temporaryDirectory
            .appendingPathComponent("archer-fake-wt-\(UUID().uuidString)", isDirectory: true)
    ...
    func testRemoveWorktreeDirectoryAllowsAlreadyGoneOrphanToClose() async {
        ...
        let source = store.workspaces[0]
        let wt = store.addWorkspace(
            workingDirectory: URL(fileURLWithPath: "/tmp/archer-missing-worktree-\(UUID().uuidString)"),
            worktreeParent: source,
            worktreeBranch: "gone"
        )
    ...
    func testUnknownAgentStatusMarkerIsIgnoredInsteadOfBecomingTitle() {
        ...
        engine(session).emitTitle("archer-agent:not-real:running")
    ...
    func testCommandFinishedKeepsTerminalTitle() {
        ...
        // just before archer's OSC 133;D fires (archer's 133 hook runs last in
    ...
    func testAddTabRespectsTemplate() {
        ...
        XCTAssertEqual(engine(session).startedConfigs.first?.environment["ARCHER_AGENT"], "claude")
    }
    ...
    func testApplyConversationIdWritesToCorrectSessionOnly() {
        ...
        // other. Same isolation we get in prod via ARCHER_SURFACE_ID routing.
        let store = makeStore()
        let ws = store.addWorkspace(workingDirectory: projectA)
        let pane = firstPane(ws)
